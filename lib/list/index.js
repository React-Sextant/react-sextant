...
const isPad = Dimensions.get('window').width>550;
...
List.Item = function(props){
    ...
    return (
        <TouchableOpacity onPress={props.onPress} disabled={!props.onPress} activeOpacity={0.7}>
                ...
                <View style={{flex:1,justifyContent:'center'}}>
                    {Array.isArray(props.children)?
                        props.children.map((a,i)=>{
                            return typeof a === 'string'?<Text style={styles.item_txt} numberOfLines={2} ellipsizeMode={'middle'} key={i}>{a}</Text>:
                                <View key={i}>{a}</View>

                        }):typeof props.children === 'string'?<Text style={styles.item_txt} numberOfLines={2}>{props.children}</Text>:
                            <View>{props.children}</View>
                    }
                </View>
                <View style={{flexDirection:'row',alignItems:'center',marginLeft:scaleSize(5)}}>
                    {typeof props.extra === 'object'?<View>{props.extra}</View>:<Text style={styles.item_extra} numberOfLines={2}>{props.extra}</Text>}
