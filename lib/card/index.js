...
export default class Card extends React.Component {
    static Title = function (props) {
        return (
            <View style={[{marginVertical:scaleSize(17,38)},props.style]}>
                {typeof props.children === "string"?
                    <Text style={{fontSize:scaleSize(16,40),color:'#333',fontWeight:'500'}}>{props.children}</Text>:
                    <React.Fragment>
                        {props.children}
                    </React.Fragment>
                }
    ...
    render(){
        ...
        return (
            <View ref={c=>{this._root = c}} style={[styles,props.style]}>
                {props.children}
            </View>
        )
    }
